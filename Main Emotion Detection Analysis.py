        ...
        #use_recorder = st.checkbox("Enable audio recorder")
         ...
         #   try:
          ...
          #      from audio_recorder_streamlit import audio_recorder
                ...
                #col1, col2 = st.columns([3, 1])
                #with col1:
                # Add some recording instructions
           #     st.markdown("""
             #   <div style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; border-left: 4px solid #3B82F6;">
            #        <p style="margin: 0;">Click the microphone button below to start recording. Click again to stop.</p>
              #      <p style="margin: 5px 0 0 0; font-size: 0.9rem;">Speak clearly and try to minimize background noise for best results.</p>
               # </div>
                #""", unsafe_allow_html=True)
                ...
                #)
                #if audio_bytes:
                 #   st.session_state['last_audio_source'] = 'record'

                    # Clear any uploaded audio from memory
                    #if 'unique_audio_uploader' in st.session_state:
                     #   st.session_state['unique_audio_uploader'] = None
                
                    # Delete any previous uploaded audio file
                  #  if os.path.exists("temp_uploaded_audio.wav"):
                   #     try:
                    #        os.remove("temp_uploaded_audio.wav")
                     #   except:
                      #      pass
                ...
                #else:
                 #   st.info("No recording detected yet. Press the microphone button to start recording.")
            ...
            #except ImportError:
             #   st.warning("Audio recorder not available. Please install with: pip install audio-recorder-streamlit")
              ...
              #  st.code("pip install audio-recorder-streamlit")
               ...
               # st.markdown("""
                #After installation, restart the application to use the audio recorder.
                #""")
