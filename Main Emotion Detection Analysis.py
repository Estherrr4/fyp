...
def app_sidebar():
    ...
    <div style="text-align: center; padding-top: 2rem; color: #94A3B8; font-size: 0.8rem;">
        ...
        <p style="margin: 0;">© 2025 Your Name</p>
    </div>
    """, unsafe_allow_html=True)
...
def audio_analysis_page():
    ...
    def check_files():
        ...
        #required_files = ['audio_sentiment_model.keras', 'encoder_label.pkl', 'scaler_data.pkl']
        #required_files = ['data/audio_sentiment_model.keras', 'data/encoder_label.pkl', 'data/scaler_data.pkl']
        required_files = [
    ...
    if missing_files:
        ...
        use_recorder = st.checkbox("Enable audio recorder")
        
        if use_recorder:
            try:
                # Import audio_recorder if available
                ...
                if audio_bytes:
                    ...
                    if st.button("Analyze Recorded Audio"):
                        with st.spinner("Analyzing your recorded audio..."):
                            emotion, confidence_scores, emotion_classes = predict_emotion("temp_recorded_audio.wav")
                            display_audio_results(emotion, confidence_scores, emotion_classes, emotion_color)
            except ImportError:
                st.warning("Audio recorder not available. Please install with: pip install audio-recorder-streamlit")
...
def display_audio_results(emotion, confidence_scores, emotion_classes, emotion_color):
    ...
    if emotion is not None:
        ...
        st.markdown('<div class="content-box">', unsafe_allow_html=True)
        st.markdown("<h2 style='color: #1E3A8A; margin-bottom: 1rem; border-bottom: 2px solid #E0E7FF; padding-bottom: 0.5rem;'>Analysis Results</h2>", unsafe_allow_html=True)
        
        # Display the emotion with matching color
        color = emotion_color.get(emotion.lower(), "#666666")
        st.markdown(f"""
        <div style="background-color: {color}; padding: 1.5rem; border-radius: 10px; border: 1px solid #E0E0E0; margin-bottom: 1rem; text-align: center;">
            <h3 style="color: #000; margin: 0; font-size: 2rem;">{emotion.upper()}</h3>
        </div>
        """, unsafe_allow_html=True)
    ...
    #st.markdown("### Or Record Audio")
     ...
     #   st.warning("Recording functionality will be implemented by your friend.")
    ...
    # Analysis button (placeholder)
     ...
     #   if audio_file:
      #      with st.spinner("Analyzing audio..."):
       ...
       #         st.info("Audio analysis will be implemented by your friend.")
                
                # Show a dummy result
        #        st.markdown(
         #           """
          #          <div style="background-color: #FFEAA7; padding: 1rem; border-radius: 0.5rem; text-align: center;">
           #             <h2 style="color: #000; margin: 0; font-size: 2rem;">JOY</h2>
            #            <p style="font-size: 1.2rem; color: #333; margin: 0.5rem 0 0 0;">Confidence: 0.85</p>
             #       </div>
              #      """, 
               #     unsafe_allow_html=True
                #)
...
def history_page():
    ...
    st.markdown('<div class="content-box">', unsafe_allow_html=True)
    st.markdown(f"<h3 style='color: #1E3A8A; margin-bottom: 1rem;'>Your Recent Analyses</h3>", unsafe_allow_html=True)
